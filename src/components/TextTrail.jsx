...
import {
  CanvasTexture, Clock, Color, LinearFilter, LinearMipmapLinearFilter,
...
const loadFont = async fam => { if ('fonts' in document) await document.fonts.load(`64px "${fam}"`) }
...
/* simplex noise (abrégé mais identique à la version fournie) */
const SIMPLEX = `
...
export default function TextTrail({
  ...
  animateColor = false,
  startColor = '#ffffff',
  textColor = undefined,          // undefined => autorise le cycle de couleur
  backgroundColor = 'transparent', // rendu transparent pour overlay sur image
  ...
  useEffect(()=>{
    ...
    if (backgroundColor === 'transparent'){
      renderer.setClearColor(new Color(0x000000), 0)   // alpha 0
    } else {
    ...
    const quadMat = new ShaderMaterial({
      uniforms:{
        sampler:{ value:null }, time:{ value:0 }, mousePos:{ value:new Vector2(-1,1) },
        noiseFactor:{ value:noiseFactor }, noiseScale:{ value:noiseScale },
        rgbPersistFactor:{ value:rgbPersistFactor }, alphaPersistFactor:{ value:alphaPersistFactor }
      },
    ...
    const labelMat = new ShaderMaterial({
      uniforms:{ sampler:{ value:null }, color:{ value:new Vector3(...persistColor.current) } },
    ...
    const drawText = () => {
      ...
      texCanvas.width = max * pixelRatio
      texCanvas.height = max * pixelRatio
    ...
    renderer.setAnimationLoop(()=>{
      ...
      renderer.setRenderTarget(rt0); renderer.clearColor(); renderer.render(fluidScene,cam); renderer.render(scene,cam)
      renderer.setRenderTarget(null); renderer.render(fluidScene,cam); renderer.render(scene,cam)
      ;[rt0,rt1] = [rt1,rt0]
    })
    ...
    return ()=>{
      ...
      renderer.dispose(); rt0.dispose(); rt1.dispose()
      quadMat.dispose(); quad.geometry.dispose(); labelMat.dispose(); label.geometry.dispose()
    }
  },[
    text,fontFamily,fontWeight,noiseFactor,noiseScale,rgbPersistFactor,alphaPersistFactor,
    animateColor,startColor,textColor,backgroundColor,colorCycleInterval,supersample
  ])
